...
CSpear::CSpear(LPDIRECT3DDEVICE9 _GraphicDevice)
...
HRESULT CSpear::NativeConstruct(void * pArg)
{
	...
	m_tagWeaponStats.szName = L"Spear";
	m_tagWeaponStats.iAttackDmg = 13;
	m_tagWeaponStats.eWeaponType = WEAPONSTATS::WEAPON_SPEAR;
...
HRESULT CSpear::Render_Cube()
{
	...
	HANDLE		hFile = CreateFile(L"../../Data/Weapon2.dat", GENERIC_READ, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
...
HRESULT CSpear::SetUp_RenderState()
{
	if (nullptr == m_pGraphic_Device)
	{
		return E_FAIL;
...
HRESULT CSpear::Release_RenderState()
{
	if (nullptr == m_pGraphic_Device)
	{
		return E_FAIL;
...
CSpear * CSpear::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CSpear* pInstance = new CSpear(pGraphic_Device);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX(TEXT("Error : Failed Created Spear Prototype"));
...
CGameObject * CSpear::Clone(void * pArg)
{
	...
	{
		MSGBOX(TEXT("Error : Failed Cloned Spear Copy"));
		Safe_Release(pInstance);
	}
	return pInstance;
}
