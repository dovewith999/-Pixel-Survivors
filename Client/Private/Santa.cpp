...
CSanta::CSanta(LPDIRECT3DDEVICE9 _pDevice)
	:CMonster(_pDevice)
{
	m_strGameObjecttag = L"Animation";
}

CSanta::CSanta(LPDIRECT3DDEVICE9 _pDevice, ST_GAMEOBJECT_FILE_INFO& tFileInfo)
	: CMonster(_pDevice)
{
...
CSanta::CSanta(const CSanta& _rhs)
	: CMonster(_rhs)
{
	m_strGameObjecttag = _rhs.m_strGameObjecttag;

	CSanta* Parent = (CSanta*)&_rhs;
	...
	if (!FAILED(Set_AniCubeInfo(m_vecAniCubeInfo[ANI_RECT_HEAD], CUBE_HEAD, L"../Bin/Resources/Ani/Santa.jpg")))
	{
	...
	if (!FAILED(Set_AniCubeInfo(m_vecAniCubeInfo[ANI_RECT_BODY], CUBE_BODY, L"../Bin/Resources/Ani/Santa.jpg")))
	{
		m_vecAniCube.push_back((CVIBuffer_Ani_Cube*)pGameInstace->Clone_Component(LEVEL_GAMEPLAY,
			TEXT("Prototype_Component_AniBody"), &m_stBodyInfo));
	}

	if (!FAILED(Set_AniCubeInfo(m_vecAniCubeInfo[ANI_RECT_LEFT_LEG], CUBE_LEFTLEG, L"../Bin/Resources/Ani/Santa.jpg")))
	{
		m_vecAniCube.push_back((CVIBuffer_Ani_Cube*)pGameInstace->Clone_Component(LEVEL_GAMEPLAY,
			TEXT("Prototype_Component_AniBody"), &m_stBodyInfo));
	}

	if (!FAILED(Set_AniCubeInfo(m_vecAniCubeInfo[ANI_RECT_RIGHT_LEG], CUBE_RIGHTLEG, L"../Bin/Resources/Ani/Santa.jpg")))
	{
		m_vecAniCube.push_back((CVIBuffer_Ani_Cube*)pGameInstace->Clone_Component(LEVEL_GAMEPLAY,
			TEXT("Prototype_Component_AniBody"), &m_stBodyInfo));
	}

	if (!FAILED(Set_AniCubeInfo(m_vecAniCubeInfo[ANI_RECT_LEFT_ARM], CUBE_LEFTARM, L"../Bin/Resources/Ani/Santa.jpg")))
	{
		m_vecAniCube.push_back((CVIBuffer_Ani_Cube*)pGameInstace->Clone_Component(LEVEL_GAMEPLAY,
			TEXT("Prototype_Component_AniBody"), &m_stBodyInfo));
	}

	if (!FAILED(Set_AniCubeInfo(m_vecAniCubeInfo[ANI_RECT_RIGHT_ARM], CUBE_RIGHTARM, L"../Bin/Resources/Ani/Santa.jpg")))
	{
		m_vecAniCube.push_back((CVIBuffer_Ani_Cube*)pGameInstace->Clone_Component(LEVEL_GAMEPLAY,
			TEXT("Prototype_Component_AniBody"), &m_stBodyInfo));
	}

	if (!FAILED(Set_AniCubeInfo(m_vecAniCubeInfo[ANI_RECT_LEFT_ELBOW], CUBE_LEFTELBOW, L"../Bin/Resources/Ani/Santa.jpg")))
	{
		m_vecAniCube.push_back((CVIBuffer_Ani_Cube*)pGameInstace->Clone_Component(LEVEL_GAMEPLAY,
			TEXT("Prototype_Component_AniBody"), &m_stBodyInfo));
	}

	if (!FAILED(Set_AniCubeInfo(m_vecAniCubeInfo[ANI_RECT_RIGHT_ELBOW], CUBE_RIGHTELBOW, L"../Bin/Resources/Ani/Santa.jpg")))
	{
		m_vecAniCube.push_back((CVIBuffer_Ani_Cube*)pGameInstace->Clone_Component(LEVEL_GAMEPLAY,
			TEXT("Prototype_Component_AniBody"), &m_stBodyInfo));
	}

	if (!FAILED(Set_AniCubeInfo(m_vecAniCubeInfo[ANI_RECT_LEFT_ANKLE], CUBE_LEFTANKLE, L"../Bin/Resources/Ani/Santa.jpg")))
	{
		m_vecAniCube.push_back((CVIBuffer_Ani_Cube*)pGameInstace->Clone_Component(LEVEL_GAMEPLAY,
			TEXT("Prototype_Component_AniBody"), &m_stBodyInfo));
	}

	if (!FAILED(Set_AniCubeInfo(m_vecAniCubeInfo[ANI_RECT_RIGHT_ANKLE], CUBE_RIGHTANKLE, L"../Bin/Resources/Ani/Santa.jpg")))
	{
		m_vecAniCube.push_back((CVIBuffer_Ani_Cube*)pGameInstace->Clone_Component(LEVEL_GAMEPLAY,
			TEXT("Prototype_Component_AniBody"), &m_stBodyInfo));
	}
...
void CSanta::Tick(_float _fDeltaTime)
{
...
D3DXMATRIX CSanta::Set_Matrix(D3DXVECTOR3 vPos, D3DXVECTOR3 vAxis, float fAngle)
{
	D3DXMATRIX Matrix;
...
void CSanta::Late_Tick(_float _fDeltaTime)
{
	...
	m_pColliderCom->Set_Position(m_vecBoneTransfrom[m_eAniState][ROOT]->Get_State(CAniTransform::ANI_TRANS_POSITION));
	...
	__super::Affected_By_Gravity(m_vecBoneTransfrom[m_eAniState][ROOT], _fDeltaTime);
	...
	case TEST_STATE_IDLE:
		if (!FAILED(Search_Target(L"Layer_Player", &m_pTarget, 100.f)))
		{
			...
			//m_vecBoneTransfrom[TEST_STATE_IDLE][NECK]->LookAt(PlayerTransform->Get_State(CTransform::STATE_POSITION));
		}
		break;
...
HRESULT CSanta::NativeConstruct_Prototype()
{
...
HRESULT CSanta::NativeConstruct_Prototype(void* pArg)
{
	if (FAILED(__super::NativeConstruct_Prototype()))
		return E_FAIL;
...
HRESULT CSanta::NativeConstruct(void* pArg)
{
...
HRESULT CSanta::SetUp_Components()
{
	CGameInstance* pGameInstace = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstace);
	...
	if (NULL == m_pRendererCom)
	{
		...
		Safe_Release(pGameInstace);
		MSGBOX(TEXT("Failed to Init CSanta"));
		return E_FAIL;
	}
	...
	/* For.Com_Collider */
	if (FAILED(__super::Add_Components(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Collider"), TEXT("Com_Collider"), (CComponent**)&m_pColliderCom, _float3(3.f, 0.f, 3.f))))
	{
...
CGameObject* CSanta::Clone(void* pArg)
{
	CSanta * pInstance = new CSanta(*this);
	...
	St_CSanta stTest;
	ZeroMemory(&stTest, sizeof(St_CSanta));
	stTest.pObject = this;
...
void CSanta::Clear_Flag(TEST_STATE eState, _float _fDeltaTime)
{
...
CGameObject* CSanta::Create(LPDIRECT3DDEVICE9 _pDevice, ST_GAMEOBJECT_FILE_INFO& stFileInfo)
{
	CSanta * pInstance = new CSanta(_pDevice);
	if (!pInstance)
		return NULL;
...
CBone* CSanta::Get_ParentBone(int index)
{
	return m_pBone[index];
}

TEST_STRUCT CSanta::Get_TestStruct(TEST_STATE eTestState)
{
	return m_vecAniInfo[eTestState];
}

HRESULT CSanta::Set_AniCubeInfo(STSHAPE CubeInfo, ANI_CUBE eCubeState, CString strFilePath)
{
...
HRESULT CSanta::Search_Target(const _tchar* pTargetTag, CGameObject** pTarget, _float fRange)
{
	CGameInstance*		pGameInstance = CGameInstance::Get_Instance();
