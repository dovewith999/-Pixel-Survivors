...
CAngryBird::CAngryBird(LPDIRECT3DDEVICE9 _pDevice)
	:CMonster(_pDevice)
{
	m_strGameObjecttag = L"Animation";
}

CAngryBird::CAngryBird(LPDIRECT3DDEVICE9 _pDevice, ST_GAMEOBJECT_FILE_INFO& tFileInfo)
	: CMonster(_pDevice)
{
...
CAngryBird::CAngryBird(const CAngryBird& _rhs)
	: CMonster(_rhs)
{
	m_strGameObjecttag = _rhs.m_strGameObjecttag;

	CAngryBird* Parent = (CAngryBird*)&_rhs;
	...
	if (!FAILED(Set_AniCubeInfo(m_vecAniCubeInfo[ANI_RECT_HEAD], CUBE_HEAD, L"../Bin/Resources/Ani/AngryBird.jpg")))
	{
	...
	if (!FAILED(Set_AniCubeInfo(m_vecAniCubeInfo[ANI_RECT_BODY], CUBE_BODY, L"../Bin/Resources/Ani/AngryBird.jpg")))
	{
		m_vecAniCube.push_back((CVIBuffer_Ani_Cube*)pGameInstace->Clone_Component(LEVEL_GAMEPLAY,
			TEXT("Prototype_Component_AniBody"), &m_stBodyInfo));
	}

	if (!FAILED(Set_AniCubeInfo(m_vecAniCubeInfo[ANI_RECT_LEFT_LEG], CUBE_LEFTLEG, L"../Bin/Resources/Ani/AngryBird.jpg")))
	{
		m_vecAniCube.push_back((CVIBuffer_Ani_Cube*)pGameInstace->Clone_Component(LEVEL_GAMEPLAY,
			TEXT("Prototype_Component_AniBody"), &m_stBodyInfo));
	}

	if (!FAILED(Set_AniCubeInfo(m_vecAniCubeInfo[ANI_RECT_RIGHT_LEG], CUBE_RIGHTLEG, L"../Bin/Resources/Ani/AngryBird.jpg")))
	{
		m_vecAniCube.push_back((CVIBuffer_Ani_Cube*)pGameInstace->Clone_Component(LEVEL_GAMEPLAY,
			TEXT("Prototype_Component_AniBody"), &m_stBodyInfo));
	}

	if (!FAILED(Set_AniCubeInfo(m_vecAniCubeInfo[ANI_RECT_LEFT_ARM], CUBE_LEFTARM, L"../Bin/Resources/Ani/AngryBird.jpg")))
	{
		m_vecAniCube.push_back((CVIBuffer_Ani_Cube*)pGameInstace->Clone_Component(LEVEL_GAMEPLAY,
			TEXT("Prototype_Component_AniBody"), &m_stBodyInfo));
	}

	if (!FAILED(Set_AniCubeInfo(m_vecAniCubeInfo[ANI_RECT_RIGHT_ARM], CUBE_RIGHTARM, L"../Bin/Resources/Ani/AngryBird.jpg")))
	{
		m_vecAniCube.push_back((CVIBuffer_Ani_Cube*)pGameInstace->Clone_Component(LEVEL_GAMEPLAY,
			TEXT("Prototype_Component_AniBody"), &m_stBodyInfo));
	}

	if (!FAILED(Set_AniCubeInfo(m_vecAniCubeInfo[ANI_RECT_LEFT_ELBOW], CUBE_LEFTELBOW, L"../Bin/Resources/Ani/AngryBird.jpg")))
	{
		m_vecAniCube.push_back((CVIBuffer_Ani_Cube*)pGameInstace->Clone_Component(LEVEL_GAMEPLAY,
			TEXT("Prototype_Component_AniBody"), &m_stBodyInfo));
	}

	if (!FAILED(Set_AniCubeInfo(m_vecAniCubeInfo[ANI_RECT_RIGHT_ELBOW], CUBE_RIGHTELBOW, L"../Bin/Resources/Ani/AngryBird.jpg")))
	{
		m_vecAniCube.push_back((CVIBuffer_Ani_Cube*)pGameInstace->Clone_Component(LEVEL_GAMEPLAY,
			TEXT("Prototype_Component_AniBody"), &m_stBodyInfo));
	}

	if (!FAILED(Set_AniCubeInfo(m_vecAniCubeInfo[ANI_RECT_LEFT_ANKLE], CUBE_LEFTANKLE, L"../Bin/Resources/Ani/AngryBird.jpg")))
	{
		m_vecAniCube.push_back((CVIBuffer_Ani_Cube*)pGameInstace->Clone_Component(LEVEL_GAMEPLAY,
			TEXT("Prototype_Component_AniBody"), &m_stBodyInfo));
	}

	if (!FAILED(Set_AniCubeInfo(m_vecAniCubeInfo[ANI_RECT_RIGHT_ANKLE], CUBE_RIGHTANKLE, L"../Bin/Resources/Ani/AngryBird.jpg")))
	{
		m_vecAniCube.push_back((CVIBuffer_Ani_Cube*)pGameInstace->Clone_Component(LEVEL_GAMEPLAY,
			TEXT("Prototype_Component_AniBody"), &m_stBodyInfo));
	}
...
void CAngryBird::Tick(_float _fDeltaTime)
{
...
D3DXMATRIX CAngryBird::Set_Matrix(D3DXVECTOR3 vPos, D3DXVECTOR3 vAxis, float fAngle)
{
	D3DXMATRIX Matrix;
...
void CAngryBird::Late_Tick(_float _fDeltaTime)
{
	...
	case TEST_STATE_IDLE:
		if (!FAILED(Search_Target(L"Layer_Player", &m_pTarget, 100.f)))
		{
			...
			m_vecBoneTransfrom[TEST_STATE_ATTACK][ROOT]->LookAt(PlayerTransform->Get_State(CTransform::STATE_POSITION), _float3(0.f, -1.f, 0.f));
		}
		break;
	case TEST_STATE_ATTACK:
		if (!FAILED(Search_Target(L"Layer_Player", &m_pTarget, 10.f)))
		{
			CTransform* PlayerTransform = ((CPlayer*)m_pTarget)->Get_PlayerTransform();
...
HRESULT CAngryBird::NativeConstruct_Prototype()
{
...
HRESULT CAngryBird::NativeConstruct_Prototype(void* pArg)
{
	if (FAILED(__super::NativeConstruct_Prototype()))
		return E_FAIL;
...
HRESULT CAngryBird::NativeConstruct(void* pArg)
{
...
HRESULT CAngryBird::SetUp_Components()
{
	CGameInstance* pGameInstace = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstace);
	...
	if (NULL == m_pRendererCom)
	{
		...
		Safe_Release(pGameInstace);
		MSGBOX(TEXT("Failed to Init CAngryBird"));
		return E_FAIL;
	}
	...
	/* For.Com_Collider */
	if (FAILED(__super::Add_Components(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Collider"), TEXT("Com_Collider"), (CComponent**)&m_pColliderCom, _float3(1.f, 1.f, 1.f))))
	{
...
CGameObject* CAngryBird::Clone(void* pArg)
{
	CAngryBird * pInstance = new CAngryBird(*this);

	if (!pInstance)
		return NULL;
	typedef struct tagCAngryBird
	{
		CAngryBird* pObject;
		TEXMONDATA eData;
	}St_CAngryBird;
	St_CAngryBird stTest;
	ZeroMemory(&stTest, sizeof(St_CAngryBird));
	stTest.pObject = this;
...
void CAngryBird::Clear_Flag(TEST_STATE eState, _float _fDeltaTime)
{
...
CGameObject* CAngryBird::Create(LPDIRECT3DDEVICE9 _pDevice, ST_GAMEOBJECT_FILE_INFO& stFileInfo)
{
	CAngryBird * pInstance = new CAngryBird(_pDevice);
	if (!pInstance)
		return NULL;
...
CBone* CAngryBird::Get_ParentBone(int index)
{
	return m_pBone[index];
}

TEST_STRUCT CAngryBird::Get_TestStruct(TEST_STATE eTestState)
{
	return m_vecAniInfo[eTestState];
}

HRESULT CAngryBird::Set_AniCubeInfo(STSHAPE CubeInfo, ANI_CUBE eCubeState, CString strFilePath)
{
...
HRESULT CAngryBird::Search_Target(const _tchar* pTargetTag, CGameObject** pTarget, _float fRange)
{
	CGameInstance*		pGameInstance = CGameInstance::Get_Instance();
